# QUIZ SECTION: String Methods Quiz Section

# QUIZ: format() Practice
# Use the coding space below to practice using the format() string method. 
# There are no right or wrong answers here, just practice!

# Write two lines of code below, each assigning a value to a variable
hi = "hello"
bye = "goodbye"

# Now write a print statement using .format() to print out a sentence and the 
#   values of both of the variables
greetings_string = "There are two ways to greet people {} and {}"
print(greetings_string.format(hi, bye))


# QUIZ: Strings Methods Coding Practice
# Below, we have a string variable that contains the first verse of the poem, If by Rudyard Kipling. 
# Remember, \n is a special sequence of characters that causes a line break (a new line).

verse = "If you can keep your head when all about you\n  Are losing theirs and blaming it on you,\nIf you can trust yourself when all men doubt you,\n  But make allowance for their doubting too;\nIf you can wait and not be tired by waiting,\n  Or being lied about, don’t deal in lies,\nOr being hated, don’t give way to hating,\n  And yet don’t look too good, nor talk too wise:"

# Use the code editor below to answer the following questions about verse and use Test Run to check your output in the quiz at the bottom of this page.

# What is the length of the string variable verse?
# What is the index of the first occurrence of the word 'and' in verse?
# What is the index of the last occurrence of the word 'you' in verse?
# What is the count of occurrences of the word 'you' in the verse?
# You will need to refer to Python's string methods documentation.

verse = "If you can keep your head when all about you\n  Are losing theirs and blaming it on you,\nIf you can trust yourself when all men doubt you,\n  But make allowance for their doubting too;\nIf you can wait and not be tired by waiting,\n  Or being lied about, don’t deal in lies,\nOr being hated, don’t give way to hating,\n  And yet don’t look too good, nor talk too wise:"
# Use the appropriate functions and methods to answer the questions above
# Bonus: practice using .format() to output your answers in descriptive messages!
len(verse)
length_string = "The length of the string is {}"
print(length_string.format(len(verse)))
verse.find('and')
verse.rfind('you')
verse.count('you')
print("The first occurance of the word 'and' has the {}th index".format(verse.find('and')))
print("The last occurance of the word 'you' has the {}th index".format(verse.rfind('you')))
print("The word 'you' occurs {} times".format(verse.count('you')))

